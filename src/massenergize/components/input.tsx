...
export const CustomSearchBar: FC<CustomSearchBarProps> = ({ placeholder, onSearchChange }) => {
    return (
        <div className="w-full relative px-3 py-2.5 flex items-center gap-1.5 text-sm bg-white border border-strokeSoft rounded-lg">
            <RiSearch2Line size={18} />
            <input 
                type="text" 
                placeholder={placeholder} 
                onChange={onSearchChange && ((e) => onSearchChange(e.target.value))} 
                className={`w-full p-0 border-none outline-none focus:outline-none`} 
            />
        </div>
    )
}
